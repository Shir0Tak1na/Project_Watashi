...
**语料库不是语言中立的**：词条要声明自己译文的语言（文件级 `"lang": "zh-CN"` 配 `"entries"` 对象，或每条词自己写 `lang`），否则一份英→中词表在被要求译成日语时会**满置信度地给出中文**。不声明即视为"任何目标语言都可用"，既有文件因此不受影响；同一个源词可以按语言各存一条。语言按语言比较（`zh` = `zh-CN` = `zho_Hans`），语料库能覆盖哪些语言会出现在 ready 事件、桌面窗口与 Web 面板里。
...
导入导出。**改出厂词条不会改写出厂文件**：它以用户层的一条同名覆盖生效，界面会告诉你它覆盖了哪条、
原来是什么、可一键还原。导入导出支持 JSON（语料库自身的写法）、CSV/TSV（表头可中文，Excel 直接打开），
